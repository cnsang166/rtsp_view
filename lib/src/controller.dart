...
class VideoViewController {
  ...
  final RtspPlayerState _videoViewState;
  _MediaControlsState _mediaControlsState;
  ...
  static Future<VideoViewController> init(
    int id,
    RtspPlayerState videoViewState,
  ) async {
    assert(id != null);
    final MethodChannel channel = MethodChannel('rtspview_$id');
  ...
  Future<void> setVideoSource(
    ...
    if (sourceType == VideoSourceType.asset) {
      ...
      await _setVideosSource(file.path, sourceType, initTime);
    } else {
      await _setVideosSource(source, sourceType, initTime);
    }
  }
  ...
  Future<void> _setVideosSource(
      String videoSource, VideoSourceType sourceType, int initTime) async {
    ...
    try {
      ...
      _videoFile = VideoFile._(source: videoSource, sourceType: sourceType, initTime: initTime);
    } catch (ex) {
      print(ex);
    }
  }
